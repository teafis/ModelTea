...
tmdl::blocks::TrigFunction2::has_error() const {
    if (input_type_1 != output_port) {
        return make_error("mismatch in input and output types");
    } else if (input_type_1 != DataType::DOUBLE &&
               input_type_1 != DataType::SINGLE) {
...
void tmdl::blocks::TrigFunction2::set_input_type(const size_t port,
                                                 const DataType type) {
    if (port == 0) {
        input_type_1 = type;
    } else if (port == 1) {
...
tmdl::DataType
tmdl::blocks::TrigFunction2::get_output_type(const size_t port) const {
    if (port == 0) {
        return output_port;
    } else {
...
template <tmdl::DataType DT, tmdl::stdlib::TrigFunction2 FCN>
...
public:
    ...
    std::shared_ptr<tmdl::BlockExecutionInterface> get_execution_interface(
        ...
        return std::make_shared<TrigExecutor>(inputValue1, inputValue2,
                                              outputValue);
    }
...
protected:
    ...
    public:
        ...
        TrigExecutor(std::shared_ptr<const tmdl::ModelValue> ptr_input_1,
                     std::shared_ptr<const tmdl::ModelValue> ptr_input_2,
...
generate_compiler_interface(const tmdl::blocks::TrigFunction2* model) {
...
tmdl::blocks::TrigATan2::get_compiled(const ModelInfo&) const {
    return generate_compiler_interface<tmdl::stdlib::TrigFunction2::ATAN2>(
        this);
}
