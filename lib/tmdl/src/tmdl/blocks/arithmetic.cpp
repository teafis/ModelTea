...
public:
    ...
    ArithCompiled(const size_t id, const size_t input_size) : _id{id}, _input_size{input_size} {
        // Empty Constructor
    }
...
protected:
    ...
    struct ArithmeticExecutor : public tmdl::BlockExecutionInterface {
        explicit ArithmeticExecutor(const std::vector<std::shared_ptr<const tmdl::ModelValue>>& inputValues,
...
tmdl::blocks::ArithmeticBase::ArithmeticBase()
    : _prmNumInputPorts(
          std::make_shared<Parameter>("num_inputs", "number of input ports", std::make_unique<ModelValueBox<DataType::UINT32>>(2))) {
    _inputTypes.resize(currentPrmPortCount(), DataType::UNKNOWN);
}

size_t tmdl::blocks::ArithmeticBase::get_num_inputs() const { return currentPrmPortCount(); }

size_t tmdl::blocks::ArithmeticBase::get_num_outputs() const { return 1; }

std::vector<std::shared_ptr<tmdl::Parameter>> tmdl::blocks::ArithmeticBase::get_parameters() const { return {_prmNumInputPorts}; }
...
void tmdl::blocks::ArithmeticBase::set_input_type(const size_t port, const DataType type) {
...
std::string tmdl::blocks::Addition::get_description() const { return "adds the provided inputs together"; }
...
std::string tmdl::blocks::Subtraction::get_description() const { return "subtracts the Multiplicationd inputs together"; }
...
std::string tmdl::blocks::Multiplication::get_description() const { return "multiplies the provided inputs together"; }
...
std::string tmdl::blocks::Division::get_description() const { return "divides the provided inputs together"; }
...
std::string tmdl::blocks::Modulus::get_description() const { return "provides the modulus of the provided inputs together"; }

std::unique_ptr<tmdl::CompiledBlockInterface> tmdl::blocks::Modulus::get_compiled(const ModelInfo&) const {
    if (has_error() != nullptr) {
