...
public:
    ...
    std::shared_ptr<tmdl::BlockExecutionInterface> get_execution_interface(
        ...
        std::shared_ptr<const tmdl::ModelValue> input_a =
            manager.get_ptr(*connections.get_connection_to(_id, 0));
        std::shared_ptr<const tmdl::ModelValue> input_b =
            manager.get_ptr(*connections.get_connection_to(_id, 1));

        auto output_value = std::dynamic_pointer_cast<
            tmdl::ModelValueBox<tmdl::DataType::BOOLEAN>>(manager.get_ptr(
            tmdl::VariableIdentifier{.block_id = _id, .output_port_num = 0}));

        return std::make_shared<RelationalExecutor>(input_a, input_b,
                                                    output_value);
    }
...
protected:
    struct RelationalComponent : public tmdl::codegen::CodeComponent {
        ...
        get_input_type() const override {
            return tmdl::codegen::InterfaceDefinition("s_in",
                                                      {"val_a", "val_b"});
        }
        ...
        std::string get_type_name() const override {
            return fmt::format("tmdl::stdlib::relational_block<{}, {}>",
    ...
    struct RelationalExecutor : public tmdl::BlockExecutionInterface {
        ...
        RelationalExecutor(
            ...
            std::shared_ptr<tmdl::ModelValueBox<tmdl::DataType::BOOLEAN>>
                outputValue)
            : output_value(outputValue) {
            input_a = std::dynamic_pointer_cast<const tmdl::ModelValueBox<DT>>(
                inputA);
            input_b = std::dynamic_pointer_cast<const tmdl::ModelValueBox<DT>>(
                inputB);

            if (output_value == nullptr || input_a == nullptr ||
                input_b == nullptr) {
                throw tmdl::ModelException(
                    "provided pointers must be non-null");
            }
        }
...
tmdl::blocks::RelationalBase::has_error() const {
    if (_inputA != _inputB) {
        return make_error("input port must have the same type");
    }

    if (const std::vector<DataType> aTypes = get_supported_types();
        std::ranges::find(aTypes, _inputA) == aTypes.end()) {
        return make_error(fmt::format(
            "relational block {} does not support provided types", get_name()));
    }

    return nullptr;
}

void tmdl::blocks::RelationalBase::set_input_type(const size_t port,
                                                  const DataType type) {
    switch (port) {
    case 0:
        _inputA = type;
...
tmdl::DataType
tmdl::blocks::RelationalBase::get_output_type(const size_t port) const {
    if (port == 0) {
        return _outputPort;
    } else {
...
std::vector<tmdl::DataType>
tmdl::blocks::RelationalNumericBase::get_supported_types() const {
    using enum DataType;
    return {
        DOUBLE,
        SINGLE,
...
std::vector<tmdl::DataType>
tmdl::blocks::RelationalEqualityBase::get_supported_types() const {
    using enum DataType;
    return {
        INT32,
        UINT32,
...
generate_compiled(const tmdl::blocks::RelationalBase* model,
                  tmdl::DataType dataType) {
    if (model->has_error() != nullptr) {
        throw tmdl::ModelException(fmt::format(
            "cannot generate a {} model with an error", model->get_name()));
    }
...
tmdl::blocks::GreaterThan::get_compiled(const ModelInfo&) const {
    return generate_compiled<tmdl::stdlib::RelationalOperator::GREATER_THAN>(
        this, get_input_type());
}
...
std::unique_ptr<tmdl::CompiledBlockInterface>
tmdl::blocks::GreaterThanEqual::get_compiled(const ModelInfo&) const {
    return generate_compiled<
        tmdl::stdlib::RelationalOperator::GREATER_THAN_EQUAL>(this,
                                                              get_input_type());
}
...
std::unique_ptr<tmdl::CompiledBlockInterface>
tmdl::blocks::LessThan::get_compiled(const ModelInfo&) const {
    return generate_compiled<tmdl::stdlib::RelationalOperator::LESS_THAN>(
        this, get_input_type());
}
...
std::unique_ptr<tmdl::CompiledBlockInterface>
tmdl::blocks::LessThanEqual::get_compiled(const ModelInfo&) const {
    return generate_compiled<tmdl::stdlib::RelationalOperator::LESS_THAN_EQUAL>(
        this, get_input_type());
}
...
std::unique_ptr<tmdl::CompiledBlockInterface>
tmdl::blocks::Equal::get_compiled(const ModelInfo&) const {
    return generate_compiled<tmdl::stdlib::RelationalOperator::EQUAL>(
        this, get_input_type());
}
...
std::unique_ptr<tmdl::CompiledBlockInterface>
tmdl::blocks::NotEqual::get_compiled(const ModelInfo&) const {
    return generate_compiled<tmdl::stdlib::RelationalOperator::NOT_EQUAL>(
        this, get_input_type());
}
