...
public:
    CompiledLimiter(const size_t id, std::optional<LimiterValues>&& constLimits) :
        _id{ id },
        _constantLimits{ std::move(constLimits) }
    {
        // Empty Constructor
    }
...
Limiter::Limiter()
{
    ...
    dynamicLimiter = std::make_shared<Parameter>(
        "dynamic_limiter",
        "Use Dynamic Limits",
        std::make_unique<ModelValueBox<BOOLEAN>>(false));

    prmMaxValue = std::make_shared<Parameter>(
        "max_value",
        "Maximum Value",
        std::make_unique<ModelValueBox<UNKNOWN>>());
