...
#ifndef MTEA_DYNVALUE_HPP
#define MTEA_DYNVALUE_HPP
...
struct ModelValue {
    ...
    virtual void copy_from(const mtea::Argument* arg) = 0;

    virtual std::unique_ptr<ModelValue> clone() const = 0;

    virtual std::string to_string() const = 0;

    virtual std::unique_ptr<mtea::Argument> to_argument() const = 0;

    virtual std::unique_ptr<mtea::Argument> to_argument_ptr() const = 0;
...
template <DataType DT> struct ModelValueBox : public ModelValue {
    ...
    std::unique_ptr<mtea::Argument> to_argument() const override { return std::make_unique<mtea::ArgumentBox<DT>>(value); }

    std::unique_ptr<mtea::Argument> to_argument_ptr() const override { return std::make_unique<mtea::ArgumentPtr<DT>>(&value); }

    void copy_from(const mtea::Argument* arg) override {
        if (auto ptr = dynamic_cast<const mtea::ArgumentBox<DT>*>(arg)) {
            value = ptr->value;
        } else {
...
template <> struct ModelValueBox<DataType::NONE> : public ModelValue {
    ...
    std::unique_ptr<mtea::Argument> to_argument() const override { return nullptr; }

    std::unique_ptr<mtea::Argument> to_argument_ptr() const override { return nullptr; }
    ...
    void copy_from(const ModelValue* in) override {
        throw ModelException(fmt::format("unable to copy from {} into an unknown data type", mtea::datatype_to_string(data_type())));
    }
