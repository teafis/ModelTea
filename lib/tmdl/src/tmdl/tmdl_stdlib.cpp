...
template <typename T> std::shared_ptr<tmdl::LibraryBlock> make_block() { return std::make_shared<T>(); }

tmdl::stdlib::StandardLibrary::StandardLibrary() {
    block_map = {{"limiter", &make_block<Limiter>}, {"cos", &make_block<TrigCos>},     {"sin", &make_block<TrigSin>},
...
std::string tmdl::stdlib::StandardLibrary::get_library_name() const { return "stdlib"; }
