...
void ConnectorObject::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    (void)widget;
    (void)option;
...
QRectF ConnectorObject::boundingRect() const { return QRectF(0.0, 0.0, std::abs(loc_b.x() - loc_a.x()), std::abs(loc_b.y() - loc_a.y())); }
...
void ConnectorObject::updateLocationValues() {
    ...
    setVisible(false);
    setPos(std::min(loc_b_scene.x(), loc_a_scene.x()), std::min(loc_b_scene.y(), loc_a_scene.y()));
...
QColor ConnectorObject::getLineColor([[maybe_unused]] QWidget* widget) const { return Qt::red; }

double ConnectorObject::getLineWidth() const { return 1.0; }
