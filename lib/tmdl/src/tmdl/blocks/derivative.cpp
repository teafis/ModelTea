...
template <tmdl::DataType DT> class CompiledDerivative : public tmdl::CompiledBlockInterface {
public:
    ...
    std::shared_ptr<tmdl::BlockExecutionInterface> get_execution_interface(const tmdl::ConnectionManager& connections,
        ...
        auto in_reset_flag = std::dynamic_pointer_cast<const tmdl::ModelValueBox<tmdl::DataType::BOOLEAN>>(
            manager.get_ptr(*connections.get_connection_to(_id, 1)));

        auto out_value = manager.get_ptr(tmdl::VariableIdentifier{.block_id = _id, .output_port_num = 0});
...
protected:
    ...
    public:
        ...
        explicit DerivativeExecutor(std::shared_ptr<const tmdl::ModelValue> input, std::shared_ptr<tmdl::ModelValue> output,
...
std::string tmdl::blocks::Derivative::get_name() const { return "derivative"; }

std::string tmdl::blocks::Derivative::get_description() const { return "derives the provided values"; }

size_t tmdl::blocks::Derivative::get_num_inputs() const { return 2; }

size_t tmdl::blocks::Derivative::get_num_outputs() const { return 1; }
...
void tmdl::blocks::Derivative::set_input_type(const size_t port, const DataType type) {
    switch (port) {
    case 0:
        input_type = type;
        break;
