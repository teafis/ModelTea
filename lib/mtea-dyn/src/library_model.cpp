...
const std::string mtea::ModelLibrary::get_library_name() const { return library_name; }

std::vector<std::string> mtea::ModelLibrary::get_block_names() const {
...
bool mtea::ModelLibrary::has_block(const std::string_view name) const { return try_get_model(name) != nullptr; }

std::unique_ptr<mtea::BlockInterface> mtea::ModelLibrary::create_block(const std::string_view name) const {
    return std::make_unique<ModelBlock>(get_model(name), get_library_name());
}

std::unique_ptr<mtea::BlockInterface> mtea::ModelLibrary::create_block(mtea::Model* model) const {
    ...
    throw mtea::ModelException(fmt::format("unable to find model matching provided model input in library {}", get_library_name()));
}

std::shared_ptr<mtea::Model> mtea::ModelLibrary::get_model(const std::string_view name) const {
...
std::shared_ptr<mtea::Model> mtea::ModelLibrary::create_new_model() {
    const auto mdl = std::make_shared<mtea::Model>();
    return add_model(mdl);
}

std::shared_ptr<mtea::Model> mtea::ModelLibrary::add_model(std::shared_ptr<Model> model) {
...
void mtea::ModelLibrary::save_model(const Model* model) {
...
void mtea::ModelLibrary::save_model(const Model* model, const std::filesystem::path& path) {
    if (!path.has_stem()) {
...
std::shared_ptr<mtea::Model> mtea::ModelLibrary::load_model(const std::filesystem::path& path) {
    if (!path.has_stem()) {
        throw ModelException(fmt::format("error setting filename '{}' due to missing stem", path.string()));
    }
...
void mtea::ModelLibrary::close_model(const mtea::Model* model) {
    if (model == nullptr) {
        return;
    }
...
void mtea::ModelLibrary::close_unused_models() {
    std::unordered_map<std::string, std::weak_ptr<mtea::Model>> weak_models;
...
std::shared_ptr<mtea::Model> mtea::ModelLibrary::try_get_model(const std::string_view name) const {
...
mtea::ModelLibrary::model_map_t::iterator mtea::ModelLibrary::find_model(const Model* mdl) {
    return std::find_if(models.begin(), models.end(), [mdl](const model_map_t::value_type& m) { return m.second.get() == mdl; });
}

mtea::ModelLibrary::model_map_t::const_iterator mtea::ModelLibrary::find_model(const Model* mdl) const {
    return std::find_if(models.cbegin(), models.cend(), [mdl](const model_map_t::value_type& m) { return m.second.get() == mdl; });
}

bool mtea::ModelLibrary::is_valid_model(const Model* mdl) {
    if (mdl == nullptr)
        return false;
