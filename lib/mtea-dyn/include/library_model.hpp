...
#ifndef MTEA_DYNMODEL_LIBRARY_HPP
#define MTEA_DYNMODEL_LIBRARY_HPP
...
public:
    ...
    [[nodiscard]] std::unique_ptr<BlockInterface> create_block(mtea::Model* model) const;

    [[nodiscard]] std::shared_ptr<Model> get_model(std::string_view name) const;

    [[nodiscard]] std::shared_ptr<Model> create_new_model();

    [[nodiscard]] std::shared_ptr<mtea::Model> add_model(std::shared_ptr<Model> model);
    ...
    void close_model(const mtea::Model* model);
...
#endif // MTEA_DYNMODEL_LIBRARY_HPP
